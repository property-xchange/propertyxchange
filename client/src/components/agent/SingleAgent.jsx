...
import PropertyFullWidth from '../property/PropertyFullWidth';
...
const SingleAgent = () => {
  ...
  useEffect(() => {
    if (agent?.listings) {
      const filtered = agent.listings.filter(
        (listing) =>
  ...
  const fetchAgent = async () => {
    try {
      setLoading(true);
      const response = await apiRequest.get(`/agent/${id}`);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4 md:mb-0">
                Properties ({agent._count.listings})
              </h2>
